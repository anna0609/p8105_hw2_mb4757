...
The cleaned dataset merge_all contains 11 variables: `r names(merge_all)`.The rows are `r nrow(merge_all)` and the columns are `r ncol(merge_all)`. The dimention is `r nrow(merge_all)` * `r ncol(merge_all)`. The range of years is `r range(merge_all$year)`.