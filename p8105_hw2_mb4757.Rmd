...
The cleaned dataset pols_month_df contains 9 variables: `r names(pols_month_df)`. The rows are `r nrow(pols_month_df)` and the columns are `r ncol(pols_month_df)`. The dimention is `r nrow(pols_month_df)` * `r ncol(pols_month_df)`. The range of years is `r range(pols_month_df$year)`.

The cleaned dataset snap_df contains 3 variables: `r names(snap_df)`. The rows are `r nrow(snap_df)` and the columns are `r ncol(snap_df)`. The dimention is `r nrow(snap_df)` * `r ncol(snap_df)`. The range of years is `r range(snap_df$year)`.

The cleaned dataset unemployment_df contains 3 variables: `r names(unemployment_df)`.The rows are `r nrow(unemployment_df)` and the columns are `r ncol(unemployment_df)`. The dimention is `r nrow(unemployment_df)` * `r ncol(unemployment_df)`. The range of years is `r range(unemployment_df$year)`.

The cleaned dataset unemployment_df contains 3 variables: `r names(merge_all)`.The rows are `r nrow(merge_all)` and the columns are `r ncol(merge_all)`. The dimention is `r nrow(merge_all)` * `r ncol(merge_all)`. The range of years is `r range(merge_all$year)`.